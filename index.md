---
layout: default
title: R. Teodoro — Public Work
description: Useful software for messy, specific problems.
---

<header class="site-header" aria-label="Site header">
  <a class="monogram" href="#top" aria-label="R. Teodoro, back to top">R/T</a>
  <nav class="site-nav" aria-label="Primary navigation">
    <a href="#work">Public projects</a>
    <a href="#method">Method</a>
    <a href="https://github.com/rteoo" rel="me">GitHub ↗</a>
  </nav>
</header>

<main id="main">
  <section class="hero" id="top" aria-labelledby="hero-title">
    <div class="hero-copy">
      <p class="eyebrow">R. Teodoro · GitHub profile · Public work</p>
      <h1 id="hero-title">Public work, <em>in the open.</em></h1>
      <p class="hero-deck">463 authored project commits across seven public project repositories—desktop tools, data libraries, extraction pipelines, and living archives.</p>
      <div class="hero-actions">
        <a class="text-link" href="#work">Explore the work</a>
      </div>
    </div>

    <div class="work-map" aria-label="Map of selected public projects">
      <p class="map-label">Contribution map / 463 project commits</p>
      <svg viewBox="0 0 700 760" role="img" aria-labelledby="map-title map-description">
        <title id="map-title">Public work map</title>
        <desc id="map-description">Five connected public projects spanning tools, data, voice, archives, and extraction.</desc>
        <path class="map-line" d="M335 380 C240 315 182 229 120 150 M335 380 C450 315 526 250 590 164 M335 380 C225 438 168 532 120 630 M335 380 C444 458 500 552 580 640 M335 380 C340 285 345 210 355 105" />
        <circle class="signal-ring" cx="335" cy="380" r="58" />
        <circle class="signal-ring" cx="335" cy="380" r="58" style="animation-delay: 1.4s" />
        <a class="map-node" href="https://github.com/rteoo/sniptype" aria-label="Sniptype public repository">
          <circle cx="335" cy="380" r="28" />
          <text x="375" y="378">sniptype</text>
          <text class="map-meta" x="375" y="396">voice / text</text>
        </a>
        <a class="map-node" href="https://github.com/rteoo/bcbpy" aria-label="bcbpy public repository">
          <circle cx="120" cy="150" r="18" />
          <text x="146" y="147">bcbpy</text>
          <text class="map-meta" x="146" y="164">public data</text>
        </a>
        <a class="map-node" href="https://github.com/rteoo/cattle-auction" aria-label="Cattle Auction public repository">
          <circle cx="590" cy="164" r="18" />
          <text x="480" y="134">cattle-auction</text>
          <text class="map-meta" x="514" y="151">video / OCR</text>
        </a>
        <a class="map-node" href="https://github.com/rteoo/rpn-calc" aria-label="RPN Calc public repository">
          <circle cx="120" cy="630" r="18" />
          <text x="146" y="628">rpn-calc</text>
          <text class="map-meta" x="146" y="645">desktop tool</text>
        </a>
        <a class="map-node" href="https://github.com/rteoo/familia-naves" aria-label="Família Naves public repository">
          <circle cx="580" cy="640" r="18" />
          <text x="432" y="620">familia-naves</text>
          <text class="map-meta" x="473" y="637">living archive</text>
        </a>
        <text x="377" y="93" fill="var(--faint)" font-family="Onest" font-size="9" letter-spacing="1.4">BUILD / TEST / RELEASE</text>
        <circle cx="355" cy="105" r="4" fill="var(--amber)" />
      </svg>
    </div>
  </section>

  <section class="contribution-section" aria-labelledby="contribution-title">
    <div class="contribution-copy reveal">
      <p class="eyebrow">Public snapshot · 01 Sep 2026</p>
      <h2 id="contribution-title"><span>463</span> authored commits on public project default branches.</h2>
      <p>Counted from GitHub’s public contributor lists. The profile repository, private repositories, and private contribution totals are intentionally excluded.</p>
    </div>
    <dl class="contribution-stats reveal">
      <div>
        <dt>Original public repositories</dt>
        <dd>08</dd>
      </div>
      <div>
        <dt>Python projects</dt>
        <dd>05</dd>
      </div>
      <div>
        <dt>Published packages</dt>
        <dd>01</dd>
      </div>
    </dl>
  </section>

  <section class="section" id="work" aria-labelledby="work-title">
    <div class="section-heading reveal">
      <span class="section-number">01 / Public repositories</span>
      <h2 id="work-title">Every original public project.</h2>
    </div>

    <div class="project-list">
      <a class="project reveal" href="https://github.com/rteoo/sniptype">
        <span class="project-index">01</span>
        <span class="project-name">sniptype</span>
        <span class="project-description">Cross-platform text expansion with rich snippets, dynamic values, and optional local voice input.</span>
        <span class="project-stat"><strong>237</strong> commits</span>
        <span class="project-arrow" aria-hidden="true">↗</span>
      </a>
      <a class="project reveal" href="https://github.com/rteoo/rpn-calc">
        <span class="project-index">02</span>
        <span class="project-name">rpn-calc</span>
        <span class="project-description">An HP 50g-style reverse Polish notation calculator with a tactile desktop interface.</span>
        <span class="project-stat"><strong>82</strong> commits</span>
        <span class="project-arrow" aria-hidden="true">↗</span>
      </a>
      <a class="project reveal" href="https://github.com/rteoo/cattle-auction">
        <span class="project-index">03</span>
        <span class="project-name">cattle-auction</span>
        <span class="project-description">A resumable pipeline that turns auction videos, speech, and screenshots into structured lot data.</span>
        <span class="project-stat"><strong>75</strong> commits</span>
        <span class="project-arrow" aria-hidden="true">↗</span>
      </a>
      <a class="project reveal" href="https://github.com/rteoo/bcbpy">
        <span class="project-index">04</span>
        <span class="project-name">bcbpy</span>
        <span class="project-description">A Python client that makes Brazilian Central Bank time series straightforward to retrieve and use.</span>
        <span class="project-stat"><strong>32</strong> commits</span>
        <span class="project-arrow" aria-hidden="true">↗</span>
      </a>
      <a class="project reveal" href="https://github.com/rteoo/novtek-site">
        <span class="project-index">05</span>
        <span class="project-name">novtek-site</span>
        <span class="project-description">The public source for Novtek’s website.</span>
        <span class="project-stat"><strong>23</strong> commits</span>
        <span class="project-arrow" aria-hidden="true">↗</span>
      </a>
      <a class="project reveal" href="https://github.com/rteoo/familia-naves">
        <span class="project-index">06</span>
        <span class="project-name">Família Naves</span>
        <span class="project-description">Eleven generations of family history organized as an explorable living archive.</span>
        <span class="project-stat"><strong>12</strong> commits</span>
        <span class="project-arrow" aria-hidden="true">↗</span>
      </a>
      <a class="project reveal" href="https://github.com/rteoo/github.io">
        <span class="project-index">07</span>
        <span class="project-name">github.io</span>
        <span class="project-description">The source for this public project profile.</span>
        <span class="project-stat">profile source</span>
        <span class="project-arrow" aria-hidden="true">↗</span>
      </a>
      <a class="project reveal" href="https://github.com/rteoo/PoC-parcelamento">
        <span class="project-index">08</span>
        <span class="project-name">PoC-parcelamento</span>
        <span class="project-description">A proof of concept for installment calculations.</span>
        <span class="project-stat"><strong>02</strong> commits</span>
        <span class="project-arrow" aria-hidden="true">↗</span>
      </a>
    </div>
  </section>

  <section class="method-section" id="method" aria-labelledby="method-title">
    <div class="section">
      <div class="section-heading reveal">
        <span class="section-number">02 / Contribution method</span>
        <h2 id="method-title">Reality first. Then code.</h2>
      </div>
    </div>
    <div class="method-grid">
      <article class="method reveal">
        <span class="method-marker">A / Observe</span>
        <h3>Find the actual friction.</h3>
        <p>Start with the stubborn part of the workflow, not the fashionable solution.</p>
      </article>
      <article class="method reveal">
        <span class="method-marker">B / Reduce</span>
        <h3>Build the narrow tool.</h3>
        <p>Cut scope until the useful thing is obvious, testable, and ready to leave the workbench.</p>
      </article>
      <article class="method reveal">
        <span class="method-marker">C / Prove</span>
        <h3>Make the state legible.</h3>
        <p>Good software should show what happened, what failed, and what to do next.</p>
      </article>
    </div>
  </section>

  <section class="closing" aria-labelledby="closing-title">
    <h2 class="reveal" id="closing-title">See the code.<br><span>Follow the signal.</span></h2>
    <footer class="closing-footer">
      <p>This page intentionally contains only work already public on GitHub.</p>
      <a class="text-link" href="https://github.com/rteoo" rel="me">github.com/rteoo</a>
    </footer>
  </section>
</main>
